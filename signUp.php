...
if (isset($_SESSION["Err"]) && $_SESSION["Err"] == 1) {
  $erreur = $_SESSION["Err"];
  echo "<script>", "alert('L\'adresse mail et déjà utiliser');", "</script>";
  $_SESSION["Err"] = 0;
} else {
  ...
  if (
    ...
    if ($mdp ===  $mdpConf) {
      try {
        ...
        $IDUser = $PhpData->InsertUser($name, $firstName, $mail, $tel, $mdp);
        echo $IDUser;
        $uri = $_SERVER['HTTP_HOST'] . '/' . substr($_SERVER['PHP_SELF'], 0, strrpos($_SERVER['PHP_SELF'], '/')) . '/';

        if ($IDUser == '') {
          $_SESSION["Err"] = 1;
          header('Location: http://' . $uri .  'signUp.php');
        } else {
          $_SESSION["Con"] = $name;
...
<body>
  <div class="newMail">
    <p>L'adresse mail et déjà utiliser</p>
  </div>

<?php
session_start();

$role = 0;
if (isset($_SESSION["Role"]) && $_SESSION["Role"] == 1) {
  $role = 1;
} else {
  $_SESSION["Role"] = '';
}

if ($role == 1) {
  try {
    $pdo = new PDO('mysql:host=localhost;dbname=quaiantique', 'root', '');

    $recupParam = $pdo->query("SELECT * FROM adminparametersindex");
  } catch (PDOException $e) {
    echo $e;
  }
?>

  <!DOCTYPE html>
  <html lang="en">

  <head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="admin.css">
    <title>Paramétre Accueil</title>
  </head>

  <body>
    <a href="admin.php" class="back"><img src="data:image/png;base64,">
    </a>

    <div class="box">
      <h1 id="param">Page Accueil</h1>
      <table id="tableParam">
        <tr>
          <th>ID</th>
          <th>Nom du Paramétre</th>
          <th>Valeur de la Paramétre</th>
          <th>Editer</th>
        </tr>

        <?php
        while ($param = $recupParam->fetch()) {
          echo "
        <tr>
          <td>{$param['ID']}</td>
          <td>{$param['ParameterName']}</td>
          <td>{$param['ParameterValue']}</td>
          <td><a class=btn id={$param['ID']} href=modifIndex.php?id={$param['ID']} ><img src=data:image/png;base64,></a>
          </td>
        </tr>";
        }
        ?>
    </div>
  </body>

  </html>
<?php
}
?>